...
import { TeamTrades, type TeamTrade } from "@/app/app/auction/team-trades";
...
export default async function TeamAuctionPage() {
  ...
  const [
    ...
    supabase
      .from("players_public")
      .select(
        "id, full_name, role, pool, nationality, is_overseas, sale_price, sold_at",
      )
  ...
  const [
    ...
    supabase
      ...
      .eq("event_edition_id", edition.id),
    // public_trades_feed rather than auction_trades: it is the curated view
    // that can name the other franchise, since teams' own RLS keeps a team
    // from joining the teams table for a rival's row.
    supabase
      ...
      .limit(25),
    // No `players(full_name)` embed, unlike the admin trade log: see the
    // players_public lookup below for why a team cannot use one.
    supabase
      .from("auction_trade_players")
      .select("trade_id, player_id, from_team_id, to_team_id, price_at_trade")
      .or(`from_team_id.eq.${user.id},to_team_id.eq.${user.id}`),
  ]);
  ...
  // Traded players are named from players_public, the curated view a team is
  // actually allowed to read — the players base table is admin-or-analytics-
  // approved only (players_select_analytics_approved, audit P0 #1), so the
  // `players(full_name)` embed the admin page uses silently returns null here.
  // Keyed on the exact ids in the legs rather than reusing `allSold`, so a
  // player whose sale was later reversed (and who is therefore no longer
  // 'sold') still shows a name instead of a dash.
  const legPlayerIds = [...new Set((tradeLegs ?? []).map((l) => l.player_id))];
  const { data: legPlayers } = legPlayerIds.length
    ? await supabase
        .from("players_public")
        .select("id, full_name")
        .in("id", legPlayerIds)
    : { data: [] as { id: string; full_name: string }[] };
  const legNames = new Map((legPlayers ?? []).map((p) => [p.id, p.full_name]));

  // Re-oriented to this team's point of view — see team-trades.tsx.
  const legsByTrade: Record<string, typeof tradeLegs> = {};
  for (const leg of tradeLegs ?? [])
    (legsByTrade[leg.trade_id] ??= []).push(leg);

  const teamTrades: TeamTrade[] = (tradeRows ?? []).map((t) => {
    const isA = t.team_a_id === user.id;
    const legs = legsByTrade[t.id] ?? [];
    const asPlayer = (leg: NonNullable<typeof tradeLegs>[number]) => ({
      id: leg.player_id,
      name: legNames.get(leg.player_id) ?? "—",
      price: leg.price_at_trade == null ? null : Number(leg.price_at_trade),
    });
    const cashOut = Number((isA ? t.cash_a_to_b : t.cash_b_to_a) ?? 0);
    const cashIn = Number((isA ? t.cash_b_to_a : t.cash_a_to_b) ?? 0);
    return {
      id: t.id,
      counterparty:
        (isA
          ? (allFranchises[t.team_b_id] ?? t.team_b_name)
          : (allFranchises[t.team_a_id] ?? t.team_a_name)) ?? "—",
      executedAt: t.executed_at,
      reversedAt: t.reversed_at,
      memo: t.memo,
      netCash: cashIn - cashOut,
      playersIn: legs.filter((l) => l.to_team_id === user.id).map(asPlayer),
      playersOut: legs.filter((l) => l.from_team_id === user.id).map(asPlayer),
    };
  });
  ...
  const squadOk =
    !ruleSet ||
    (players.length >= ruleSet.min_squad_size &&
      players.length <= ruleSet.max_squad_size);
  ...
  const roleGroups = Array.from(byRole.entries()).sort(
    (a, b) => roleRank(a[0]) - roleRank(b[0]),
  );

  const roleLimits = (ruleSet?.role_limits ?? {}) as Record<
    string,
    { max?: number } | undefined
  >;
  const mostExpensive = players.reduce<(typeof players)[number] | null>(
    (best, p) =>
      Number(p.sale_price ?? 0) > Number(best?.sale_price ?? 0) ? p : best,
    null,
  );
      ...
      <div className="mx-auto w-full max-w-3xl space-y-8 px-6 pb-12 pt-12">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatTile
            ...
            value={<Money value={balanceRow?.balance ?? 0} />}
            tone="gold"
          />
          <StatTile label="Spent" value={<Money value={spend} />} />
          <StatTile
            label={
              ruleSet ? `Squad (min ${ruleSet.min_squad_size})` : "Squad size"
            }
          ...
          <StatTile
            label={
              ruleSet ? `Overseas (max ${ruleSet.max_overseas})` : "Overseas"
            }
            value={overseasCount}
          ...
          <div className="flex items-baseline justify-between border-b border-border pb-2">
            ...
            {mostExpensive && (
              <span className="text-xs text-ink-3">
                Marquee buy: {mostExpensive.full_name} ·{" "}
                <Money
                  value={Number(mostExpensive.sale_price ?? 0)}
                  className="text-xs"
                />
              </span>
            )}
              ...
              <div className="space-y-2 pt-1">
                {Object.entries(roleLimits).map(([role, limit]) => (
                  <MeterBar
                    ...
                    max={limit?.max ?? 0}
                    tone={
                      limit?.max && (roleCounts[role] ?? 0) > limit.max
                        ? "danger"
                        : "analytics"
                    }
                  />
