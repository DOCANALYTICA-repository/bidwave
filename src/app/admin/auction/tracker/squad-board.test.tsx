...
// registers itself — same note as team-card.test.tsx.
afterEach(cleanup);

function player(id: string, name: string, salePrice: number): SquadPlayer {
  return {
    id,
    name,
    role: "BATTER",
    ...
    salePrice,
    soldAt: null,
    realisation: salePrice / 20_000_000,
  };
}

function team(over: Partial<TeamTracker> & { teamId: string }): TeamTracker {
  return {
    name: "Registered Name",
    franchise: "KOCHI TUSKERS KERALA",
    rank: 1,
    purse: { funded: 1_250_000_000, playerSpend: 335_000_000, analyticsSpend: 0, balance: 915_000_000 },
    squad: [],
    squadSize: 0,
    overseasCount: 0,
    slotsToMinimum: 18,
...
describe("SquadBoard", () => {
  ...
  it("lists each player with the price paid", () => {
    ...
    const rahul = screen.getByText("KL RAHUL").closest("li")!;
    expect(within(rahul).getByText("₹23cr")).toBeInTheDocument();
    const shami = screen.getByText("MOHAMMAD SHAMI").closest("li")!;
    expect(within(shami).getByText("₹10.5cr")).toBeInTheDocument();
  });
  ...
  it("keeps each squad list scrollable so a full 23-man roster cannot push the grid", () => {
    ...
    render(<SquadBoard teams={[team({ teamId: "t1", squad: full, squadSize: 23 })]} />);
