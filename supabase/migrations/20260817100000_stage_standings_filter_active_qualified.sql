-- ---------------------------------------------------------------------------
-- Fix: stage_standings() ranked every team ever registered in the edition
-- (97), not just the teams still active and qualified for this stage (35
-- after r1_r2 eliminations). The `weighted` CTE (introduced by
-- 20260814050000_quiz_retest_round.sql, kept by
-- 20260815060000_fix_stage_standings_empty_stage.sql) scans
-- `public.teams` filtered only by `event_edition_id` — it never checked
-- `teams.status` or the prior stage's `qualifications.decision`, so
-- disqualified/eliminated teams kept appearing (and getting scored) in
-- later stages' admin scoring tab.
--
-- Fix reuses the same eligibility semantics already enforced for
-- auction/simulation/analytics access by
-- public.team_meets_stage_requirement() (see
-- 20260801093000_auction_integrity_and_qualification.sql): a team is
-- excluded from a stage's standings if it is not `active`, or if any round
-- contributing to this stage requires qualification from a prior stage and
-- the team's decision there was not 'qualified'.
--
-- Everything else (empty-stage guard, supersede handling, tie-break rules)
-- is unchanged from 20260815060000.
-- ---------------------------------------------------------------------------
...
begin
  ...
  weighted as (
    ...
    --
    -- Team universe is additionally restricted to teams still eligible for
    -- THIS stage: status = 'active', and not excluded by any contributing
    -- round's qualification gate (team_meets_stage_requirement mirrors the
    -- same check already used to gate auction/simulation/analytics access).
    ...
    where t.event_edition_id = v_event_edition_id
      and t.status = 'active'
      and not exists (
        select 1 from round_scores rs
        where not public.team_meets_stage_requirement(rs.round_id, t.id)
      )
    group by t.id
  ),
  adjustments as (
